...
public class LC127_WordLadder {
    ...
    // 会陷入循环，导致无法退出，beginWord = "hit"，endWord = "cog"，wordList = ["hot","dot","tog","cog"]
    private static int ladderLengthMy1(String beginWord, String endWord, List<String> wordList) {
        ...
        while (!map.isEmpty()) {
            ...
            for (String word : map.get(count)) {
                if (word.equals(endWord)) {
                    return count;
                }
                for (String str : wordList) {
                    if (isLadderWord(word, str)) {
                        hasLadder = true;
                        List<String> cache = map.getOrDefault(count + 1, new ArrayList<>());
                        cache.add(str);
                        map.put(count + 1, cache);
                    }
                }
            }
            if (!hasLadder) {
    ...
    // 时间复杂度太高，无法处理过长的情况
    private static int ladderLengthMy2(String beginWord, String endWord, List<String> wordList) {
        if (!wordList.contains(endWord)) {
            return 0;
        }
        int count = 1;
        ...
        while (!map.isEmpty()) {
            ...
            for (String word : map.get(count)) {
                ...
                for (String str : wordList) {
                    if (!usedWordSet.contains(str) && isLadderWord(word, str)) {
                        hasLadder = true;
                        List<String> cache = map.getOrDefault(count + 1, new ArrayList<>());
                        cache.add(str);
                        map.put(count + 1, cache);
    ...
    private static boolean isLadderWord(String word, String str) {
        char[] wordChars = word.toCharArray();
        char[] strChars = str.toCharArray();
        int count = 0;
        for (int i = 0; i < wordChars.length; i++) {
            if (wordChars[i] != strChars[i]) {
                count++;
            }
        }
        return count == 1;
    ...
    private static int ladderLengthAnswer1(String beginWord, String endWord, List<String> wordList) {
        ...
        while (!queue.isEmpty()) {
            int size = queue.size();
            for (int i = 0; i < size; i++) { // 每次都把queue清空，就可以避免使用map了
                ...
                for (int j = 0; j < word.length(); j++) {
                    for (int k = 'a'; k <= 'z'; k++) { // 有意思，只要26次，就可以得到所有的变化
